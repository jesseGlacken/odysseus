...
Moved from src/session_search.py to src/domain/auth/session_search.py (ODY-19 / P2.2).
...
def _context_for_message(db, msg: DBChatMessage, count: int) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    return [_message_to_context(row) for row in reversed(before_rows)], [_message_to_context(row) for row in after_rows]
...
def _search_like(db, query, limit, owner, include_archived, context_messages, restrict_owner, include_legacy_owner):
...
def _search_fts(db, query, limit, owner, include_archived, context_messages, restrict_owner, include_legacy_owner):
...
def search_session_messages(
    ...
    if owns_db:
        db = SessionLocal()
    try:
        fts_results = _search_fts(db, query, limit, owner, include_archived, context_messages, restrict_owner, include_legacy_owner)
        if fts_results is not None:
            like_results = _search_like(db, query, limit, owner, include_archived, context_messages, restrict_owner, include_legacy_owner)
            ...
            for result in [*fts_results, *like_results]:
                ...
                if len(merged) >= limit:
                    break
            return merged
        return _search_like(db, query, limit, owner, include_archived, context_messages, restrict_owner, include_legacy_owner)
    finally:
        if owns_db:
            db.close()
