...
Moved from src/auth_helpers.py to src/domain/auth/auth_helpers.py (ODY-19 / P2.2).
...
def require_user(request: Request) -> str:
    """FastAPI dependency: reject unauthenticated callers."""
    if _is_api_token_request(request):
...
def owner_filter(query, model_cls, user: str, *, include_shared: bool = True):
    """Filter `query` so only rows owned by `user` (and optionally null-owner 'shared' rows) come through."""
    if not user:
        return query
    if include_shared:
