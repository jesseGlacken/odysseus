"""CalDAV write-back: push local create/update/delete out to the remote.

Moved from src/caldav_writeback.py to src/domain/calendar/caldav_writeback.py (ODY-19 / P2.2).
The original location keeps a backward-compat shim.

Import change: lazy `from src.caldav_sync import _stable_cal_id` → `from .caldav_sync import _stable_cal_id`.
"""
...
def _stable_cal_id(remote_url: str, owner: str = "", account_id: str = "") -> str:
    ...
    # caldav_sync.py is large and deferred to a follow-up PR; import via the legacy flat path.
...
def push_event(calendars, local_cal_id: str, ev: dict, *, delete: bool = False,
    ...
    if delete:
        ...
        existing.delete()
        return {"ok": True, "calendar_url": remote_url, "remote_href": _resource_href(existing), "remote_etag": _resource_etag(existing)}
    ical = build_event_ical(ev)
    if existing is not None:
        existing.data = ical
        existing.save()
        return {"ok": True, "updated": True, "calendar_url": remote_url, "remote_href": _resource_href(existing), "remote_etag": _resource_etag(existing)}
    created = remote.save_event(ical)
    return {"ok": True, "created": True, "calendar_url": remote_url, "remote_href": _resource_href(created), "remote_etag": _resource_etag(created)}
...
def _persist_writeback_result(owner: str, calendar_id: str, uid: str, result: dict, *, delete: bool) -> None:
    ...
    db = SessionLocal()
    try:
        calendar = db.query(CalendarCal).filter(CalendarCal.id == calendar_id, CalendarCal.owner == owner).first()
        if calendar and result.get("calendar_url"):
            calendar.caldav_base_url = result.get("calendar_url")
        if delete:
            tombstone = db.query(CalendarDeletedEvent).filter(CalendarDeletedEvent.uid == uid, CalendarDeletedEvent.owner == owner).first()
            if result.get("ok"):
                if tombstone:
                    db.delete(tombstone)
            elif tombstone:
...
async def writeback_event(owner: str, calendar_source: str, calendar_id: str,
    ...
    try:
        from src.caldav_sync import _load_caldav_accounts, validate_caldav_url
