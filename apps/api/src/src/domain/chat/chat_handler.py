# src/domain/chat/chat_handler.py
"""Handler for chat endpoint operations.

Moved from src/chat_handler.py to src/domain/chat/chat_handler.py (ODY-19 / P2.2).
The original location keeps a backward-compat shim.

Import change: `from src.chat_helpers import ...` → relative import within chat domain.
"""
import os
import asyncio
import logging
...
from .chat_helpers import extract_urls, model_supports_vision
...
class ChatHandler:
    ...
    def __init__(self, session_manager, memory_manager, chat_processor, research_handler, preset_manager, upload_handler):
    ...
    def validate_and_extract_preset(self, preset_id: Optional[str]) -> tuple:
        ...
        if preset_id and preset_id in self.preset_manager.presets:
            ...
            if character_name:
                ...
                preset_system_prompt = f"{name_line} {preset_system_prompt}" if preset_system_prompt else name_line
    ...
    async def preprocess_message(
        ...
        if not vision_enabled and isinstance(user_content, list):
            text_parts = [item.get("text", "") for item in user_content if isinstance(item, dict) and item.get("type") == "text"]
        ...
        if isinstance(user_content, list):
            text_for_context = next(
                (item["text"] for item in user_content if item.get("type") == "text"), enhanced_message,
            )
        else:
