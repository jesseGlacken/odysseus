# src/domain/documents/document_processor.py
"""Document processing: PDF/OCR extraction, text file handling, image VL analysis, user content building.

Moved from src/document_processor.py to src/domain/documents/document_processor.py (ODY-19 / P2.2).
The original location keeps a backward-compat shim.

Import note: `from src.llm_core import llm_call` is unchanged (cross-domain; shim handles it).
"""
...
def _process_text_file(path: str) -> str:
    language_map = {
        ...
        ".php": "php", ".rb": "ruby", ".ts": "typescript", ".jsx": "javascript", ".tsx": "typescript",
    }
...
def _fit_inline_attachment_text(text: str, remaining: int, display_name: str) -> tuple[str, int]:
    text = text or ""
...
def _process_office_document(path: str, display_name: str, session_id: str | None = None,
                              auto_opened_docs: list[Dict[str, Any]] | None = None, owner: str | None = None) -> str:
    from src.markitdown_runtime import is_markitdown_format, convert_to_markdown, load_markitdown
        ...
        if session_id:
            try:
                ...
                doc_id = create_office_document(
                    session_id=session_id, upload_id=os.path.basename(path), title=title, body_text=markdown,
                )
                if doc_id and auto_opened_docs is not None:
                    ...
                    try:
                        ...
                        if _d:
                            auto_opened_docs.append({"doc_id": _d.id, "title": _d.title, "language": _d.language, "content": _d.current_content, "version": _d.version_count})
                    finally:
                        _db.close()
            except Exception as e:
...
def _resolve_vl_model(configured: str, owner: str | None = None) -> tuple:
    ...
    candidates = [
        ...
        "gemini-2.0-flash", "gemini-2.5-pro", "llava", "pixtral", "qwen2-vl",
    ]
...
def build_user_content(
    ...
    for fid in attachment_ids or []:
        ...
        if upload_handler.is_image_file(display_name, mime):
            try:
                ...
                content.append({"type": "image_url", "image_url": {"url": f"data:image/{image_format};base64,{encoded_string}"}})
            except Exception as e:
            ...
            try:
                ...
                content.append({"type": "audio", "audio": {"url": f"data:audio/{audio_format};base64,{encoded_string}"}})
            except Exception as e:
            ...
            if mime == "application/pdf":
                ...
                if session_id:
                    try:
                        ...
                        from src.pdf_form_doc import save_field_sidecar, create_form_markdown_document, create_plain_pdf_document
                        ...
                        if is_form:
                            ...
                            doc_id = create_form_markdown_document(session_id=session_id, fields=fields, upload_id=os.path.basename(path), title=title, intro_text=pdf_body_text)
                            if doc_id:
                                extracted_text = f"\n\n[Form attached: {title} — {len(fields)} fields. Opened in editor — edit the values there and use the Export PDF button when done.]"
                                if body_for_chat:
                                    extracted_text += f"\n\n[PDF content — {title}]:\n{body_for_chat}{truncated_marker}"
                        else:
                            doc_id = create_plain_pdf_document(session_id=session_id, upload_id=os.path.basename(path), title=title, body_text=pdf_body_text)
                            if doc_id:
                ...
                extracted_text = _process_office_document(path, display_name, session_id=session_id, auto_opened_docs=auto_opened_docs, owner=owner)

            extracted_text, inline_attachment_remaining = _fit_inline_attachment_text(extracted_text, inline_attachment_remaining, display_name)
            if content and content[0]["type"] == "text":
    ...
    if not has_media and content:
        combined_text = "".join(item.get("text", "") for item in content if isinstance(item, dict) and item.get("type") == "text")
