"""Model discovery: scan local network / Tailscale for OpenAI-compatible endpoints.

Moved from src/model_discovery.py to src/domain/cookbook/model_discovery.py (ODY-19 / P2.2).
...
class ModelDiscovery:
    ...
    def _fingerprint_provider(self, host: str, port: int) -> Optional[str]:
        try:
            ...
            if r.is_success:
                ...
                if (isinstance(models, list) and models and isinstance(models[0], dict) and "key" in models[0] and "architecture" in models[0]):
                    return "lmstudio"
        except Exception:
            pass
        try:
            ...
            if r.is_success:
                props = r.json() or {}
                if isinstance(props, dict) and ("default_generation_settings" in props or "total_slots" in props or "chat_template" in props):
                    return "llamacpp"
        except Exception:
            pass
        return None
    ...
    def get_providers(self) -> Dict[str, Any]:
        ...
        if self.openai_api_key:
            openai_models = ["gpt-5.2-codex", "gpt-4o-mini", "gpt-image-1.5", "gpt-4o", "gpt-5.2", "gpt-5.2-pro"]
            providers.append({"provider": "openai", "items": [{"url": "https://api.openai.com/v1/chat/completions", "models": openai_models}]})
