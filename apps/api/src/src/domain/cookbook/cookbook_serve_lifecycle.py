"""Cookbook serve lifecycle: kills scheduler-owned serves whose end-of-window has passed.

Moved from src/cookbook_serve_lifecycle.py to src/domain/cookbook/cookbook_serve_lifecycle.py (ODY-19 / P2.2).
...
async def _delete_endpoint_for_task(task: dict) -> None:
    ...
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            r = await client.get(f"{internal_api_base()}/api/model-endpoints", headers=_internal_headers())
            if r.status_code >= 400:
                return
            eps = r.json() if r.content else []
            ep = next((e for e in eps if e.get("id") == endpoint_id), None)
            if ep:
                await client.delete(f"{internal_api_base()}/api/model-endpoints/{ep['id']}", headers=_internal_headers())
                logger.info(f"cookbook_serve_lifecycle: deleted endpoint {ep.get('id')} ({ep.get('base_url')}) after scheduled stop")
...
async def _stop_serve(session_id: str, remote_host: str = "", ssh_port: str = "") -> bool:
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            r = await client.post(f"{internal_api_base()}/api/shell/exec", json={"command": cmd}, headers=_internal_headers())
            if r.status_code >= 400:
                return False
