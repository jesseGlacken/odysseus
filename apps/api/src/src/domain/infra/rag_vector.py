...
Moved from src/rag_vector.py to src/domain/infra/rag_vector.py (ODY-19 / P2.2).
            ...
            if new_texts:
                ...
                for i in range(0, len(new_texts), 100):
                    try:
                        lane.collection.add(
                            ids=new_ids[i:i + 100],
                            embeddings=lane.encode(new_texts[i:i + 100]),
                            documents=new_texts[i:i + 100],
    ...
    def rename_owner(
        ...
        *,
        ...
        for lane_name, collection in self._collections_for_delete():
            try:
                results = collection.get(where={"owner": old_owner}, include=["metadatas"])
            except Exception as e:
                ...
                for fname in files:
                    ...
                    try:
                        ...
                        if not content or not content.strip():
                            continue
                        meta = {'source': fpath, 'filename': fname, 'directory': root, 'type': ext}
                        if owner:
                            meta['owner'] = owner
                        ...
                        failed += 1
            return {'success': True, 'indexed_count': indexed, 'failed_count': failed,
