...
Moved from src/runtime_paths.py to src/domain/infra/runtime_paths.py (ODY-19 / P2.2).
...
def get_app_root() -> str:
    ...
    NOTE: This file moved from src/runtime_paths.py (2 dirname levels) to
    src/domain/infra/runtime_paths.py (4 dirname levels).  Both resolve to
    the same apps/api/src/ directory.
    """
    if getattr(sys, "frozen", False):
        return getattr(sys, "_MEIPASS", os.path.dirname(os.path.abspath(sys.executable)))
    # 4 levels up: infra/ → domain/ → src/ → apps/api/src/ (== app root)
