...
COPY apps/api/requirements.txt apps/api/requirements-optional.txt ./
RUN pip install --no-cache-dir -r apps/api/requirements.txt \
    && if [ "$INSTALL_OPTIONAL" = "true" ]; then pip install --no-cache-dir -r apps/api/requirements-optional.txt; fi
...
CMD ["uvicorn", "apps.api.src.app:app", "--host", "0.0.0.0", "--port", "7000"]
